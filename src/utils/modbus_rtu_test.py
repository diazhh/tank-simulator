"""
Utility module for testing Modbus RTU communication with the simulator.
"""
import os
import sys
import time
...
class ModbusRTUTest:
    ...
    Attributes:
        client (ModbusSerialClient): Modbus RTU client
        config (Dict): Communication configuration
    """
    
    def __init__(self, config_path: str):
        """
        Initialize the Modbus RTU test client.
    ...
    def disconnect(self) -> None:
        """Disconnect from the Modbus RTU server."""
        self.client.close()
    
    def read_radar_data(self, modbus_address: int) -> Dict:
        """
        Read radar data from the Modbus RTU server.
        ...
        for register_name, offset in offsets.items():
            ...
            if data_type == 'uint32':
                ...
                response = self.client.read_holding_registers(
                    address=address,
                    count=2,
                    slave=self.config['modbus']['server']['unit_id']
                )
    ...
    def write_radar_config(self, modbus_address: int, register_name: str, value: int) -> bool:
        """
        Write radar configuration to the Modbus RTU server.
...
def main():
    """Main entry point for Modbus RTU test utility."""
    ...
    # Create Modbus RTU test client
    test_client = ModbusRTUTest(args.config)
    
    # Connect to server
    print(f"Connecting to Modbus RTU server...")
    if not test_client.connect():
        print("Error: Failed to connect to Modbus RTU server")
        sys.exit(1)
